...
pub fn get_workflow_steps(conn: &Connection, todo_id: i64) -> Result<Vec<WorkflowStep>> {
    let mut stmt = conn.prepare(
        "SELECT id, todo_id, step_order, step_type, subtask_id, prompt_text, status, created_at
...
pub fn set_workflow_steps(
    ...
    let mut stmt = conn.prepare(
        "INSERT INTO workflow_steps (todo_id, step_order, step_type, subtask_id, prompt_text)
         VALUES (?1, ?2, ?3, ?4, ?5)"
    )?;
