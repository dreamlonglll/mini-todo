...
6. **（实现期补充）底色/透明度仅在深色主题开启时生效**，未开启时两个控件置灰。
   浅色主题是不透明白底，透明度无从谈起；若让底色在浅色主题下也生效，会出现
   「深底色 + 深色文字」不可读的组合。代价是深色主题默认关闭，用户第一次打开
   设置会看到两个灰控件。
...
- 决策 6 让「界面底色」在默认状态下不可用，可发现性差；若后续要放开到浅色主题，
  需要一并解决文字色与底色亮度的搭配问题（例如按底色亮度自动推导文字明暗）。
- 样式变量化把原先 5 层半透明黑叠加压成 #app 单层，默认 alpha 必须取合成值 0.45
  而非任一层的 0.15，否则老用户升级后窗口会明显变透（code-review 发现）。
