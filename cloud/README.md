...
> 当前进度：**PR1 + PR2 完成**——只读骨架、REST CRUD、dirty flag push、配套
> Claude Code Skill 全部就位。PC 端的条件 PUT / 412 重试 / per-record merge 在
> PR3 实现。
...
- [ ] PC 端条件 PUT + 412 重试 + per-record merge（PR3，修改 `pc/src-tauri/src/services/webdav.rs` + `pc/src-tauri/src/commands/sync_cmd.rs`）
- [ ] PC 端新增 v24 migration（`webdav_last_modified` setting）
