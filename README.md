...
- 待办描述使用 Markdown 编辑器（Milkdown），支持粘贴/拖拽上传图片
- 描述支持粘贴 Markdown 源码自动解析为富文本、GFM 语法（表格/任务清单/删除线），并提供源码/预览分栏的放大编辑弹窗（可最大化）
- 点击待办默认进入只读详情：简化排版突出内容，展示标题、Markdown 渲染的描述、通知状态与优先级，可一键切换到编辑模式
